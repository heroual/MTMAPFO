...
import { PlusCircle, Search, Server, Network, Box, Filter, Edit2, Trash2, Building, Clock, AlertTriangle, FileSpreadsheet, Cable, Link as LinkIcon } from 'lucide-react';
...
const EquipmentsPage: React.FC = () => {
  ...
  const { olts, splitters, pcos, sites, msans, cables, joints, chambers, deleteEquipment, deleteCable } = useNetwork();
  const [activeTab, setActiveTab] = useState<'ALL' | 'SITE' | 'OLT' | 'SPLITTER' | 'PCO' | 'CABLE' | 'JOINT'>('ALL');
  ...
  const getAllItems = () => {
    ...
    if (activeTab === 'ALL' || activeTab === 'JOINT') items.push(...joints, ...chambers);
  ...
  const handleDeleteConfirm = () => {
      ...
      if (deleteEntity.type === EquipmentType.CABLE) {
          deleteCable(deleteEntity.id);
      } else {
    ...
    <div className="p-4 md:p-8 h-full flex flex-col relative">
      ...
      <div className="glass-panel p-4 rounded-xl border border-slate-200 dark:border-slate-700 mb-6 flex flex-col md:flex-row items-center gap-4">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-900/50 rounded-lg p-1 border border-slate-200 dark:border-slate-700 w-full md:w-auto overflow-x-auto">
          {['ALL', 'SITE', 'OLT', 'SPLITTER', 'PCO', 'CABLE', 'JOINT'].map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === 'JOINT' ? 'JOINTS' : t(`inventory.tabs.${tab}`)}
            </button>
          ))}
        </div>
      </div>
                ...
                <div key={item.id} className="grid grid-cols-12 gap-4 p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors items-center border border-transparent hover:border-slate-200 dark:hover:border-slate-700 group">
                  <div className="col-span-4 flex items-center gap-3">
                    <div className={`p-2 rounded-lg border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm`}>
                      ...
                      {(item.type === EquipmentType.JOINT || item.type === EquipmentType.CHAMBER) && <LinkIcon size={18} className="text-amber-600 dark:text-amber-400" />}
                    </div>
                    <div>
