...
`;
  ...
  data.sites.forEach(site => {
    kml += `      <Placemark>
        ...
        <Point><coordinates>${site.location?.lng || 0},${site.location?.lat || 0},0</coordinates></Point>
      </Placemark>\n`;
  });
  kml += `    </Folder>\n`;
  ...
  data.joints.forEach(joint => {
    // Fix: jointType and capacityFibers are stored in metadata
    const jointType = joint.metadata?.jointType || 'DOME';
    const capacityFibers = joint.metadata?.capacityFibers || 0;
    kml += `      <Placemark>
        <name>${esc(joint.name)}</name>
        <description>Type: ${esc(String(jointType))}, Capacity: ${capacityFibers}F</description>
        <styleUrl>#style_joint</styleUrl>
        <Point><coordinates>${joint.location?.lng || 0},${joint.location?.lat || 0},0</coordinates></Point>
      </Placemark>\n`;
  });
  kml += `    </Folder>\n`;
  ...
  data.pcos.forEach(pco => {
    kml += `      <Placemark>
        ...
        <Point><coordinates>${pco.location?.lng || 0},${pco.location?.lat || 0},0</coordinates></Point>
      </Placemark>\n`;
  });
  kml += `    </Folder>\n`;
