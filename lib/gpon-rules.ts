...
export const GponRules = {
  ...
  canAddSlot: (olt: OLT, currentSlots: Slot[]): boolean => {
    // Fix: totalSlots is stored in metadata, not a direct property of OLT
    const totalSlots = (olt as any).metadata?.totalSlots || 0;
    return currentSlots.filter(s => s.oltId === olt.id).length < totalSlots;
  },
