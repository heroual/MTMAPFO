...
const GlobalSearch: React.FC<GlobalSearchProps> = ({ onSelectResult }) => {
  ...
  const performSearch = async (value: string) => {
    ...
    const eqResults = equipments.filter(eq => {
      ...
      const logicalPath = (eq.logicalPath || '').toLowerCase();
      const id = (eq.id || '').toLowerCase();
      return name.includes(term) || logicalPath.includes(term) || id.includes(term);
    }).map(eq => ({
      id: eq.id,
      label: eq.name || 'Équipement sans nom',
        ...
        if (pco.ports) {
            pco.ports.forEach(port => {
                if (port.client) {
                    ...
                    const ontSerial = (c.ontSerial || '').toLowerCase();
                    
                    if (name.includes(term) || login.includes(term) || ontSerial.includes(term)) {
                        clientResults.push({
                            id: c.id,
                            label: c.name || 'Client sans nom',
                            subLabel: `${c.login || 'N/A'} - ${pco.name || 'PCO'} (Port ${port.id})`,
                            category: 'CLIENT',
                            pcoId: pco.id,
    ...
    if (term.length >= 3) {
        try {
            ...
            const formattedAddresses = addressResults.map((addr: any) => ({
                ...
                subLabel: addr.label || 'Adresse inconnue',
                category: 'ADDRESS',
