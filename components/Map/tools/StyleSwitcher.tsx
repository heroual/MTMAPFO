...
const StyleSwitcher: React.FC<StyleSwitcherProps> = ({ map }) => {
  ...
  return (
    <div className="relative group" onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)}>
      <button className="glass-panel p-2 rounded-lg border border-slate-200 dark:border-slate-700 shadow-xl text-slate-500 hover:text-iam-red dark:text-slate-400 dark:hover:text-white transition-colors bg-white dark:bg-slate-900/80">
        <Layers size={20} />
      </button>

      <div className={`absolute right-full top-0 mr-2 glass-panel p-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl w-48 transition-all duration-200 origin-right bg-white dark:bg-slate-900/90 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
